...
#ifndef __INET_PACKET_WAVE_H_
#define __INET_PACKET_WAVE_H_
 ...
 */
class INET_API PacketWAVE : public cPacket
{
...
//    const Chunk *getContent() const { return content.get(); } // only for class descriptor
  ...
  public:
    ...
    explicit PacketWAVE(const char *name = nullptr, short kind = 0);
    PacketWAVE(const char *name, const Ptr<const Chunk>& content);
    PacketWAVE(const PacketWAVE& other);

    virtual PacketWAVE *dup() const override { return new PacketWAVE(*this); }
     ...
     */
    void copyTags(const PacketWAVE& source) {
...
inline std::ostream& operator<<(std::ostream& os, const PacketWAVE *packet) { return os << packet->str(); }
