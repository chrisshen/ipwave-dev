...
//            par("address").setStringValue(inet::MacAddress::generateAutoAddress().str().c_str());
//            std::ostringstream oSS; oSS << par("address").stringValue();
//            addressString = oSS.str().c_str();
//        }
		myMacAddr = inet::MacAddress::generateAutoAddress();

//		myMacAddr.formInterfaceIdentifier();

		std::cout << "myMacAddr: " << myMacAddr << std::endl;
//        myMacAddr.setAddress("0A-AA-00-00-00-01");
...
//		WaveShortMessage* pktToSend = myEDCA[activeChannel]->initiateTransmit(lastIdle);
		cPacket* pktRaw = myEDCA[activeChannel]->initiateTransmit(lastIdle);
		lastCPkt = pktRaw;

		WSM* pktToSend_WSM = nullptr;
		inet::Packet* pktToSend_IP = nullptr;

        Mac80211Pkt* mac = new Mac80211Pkt(pktRaw->getName(), pktRaw->getKind());

		if ((pktToSend_WSM = dynamic_cast<WSM*>(pktRaw)) != NULL){
		    ASSERT(pktToSend_WSM);
	        lastAC = mapUserPriority(pktToSend_WSM->getUserPriority());
	        lastWSM = pktToSend_WSM;

	        //send the packet
	        if (pktToSend_WSM->getRecipientAddress() != MacAddress::UNSPECIFIED_ADDRESS) {
	            mac->setDestAddr(pktToSend_WSM->getRecipientAddress());
	        }
	        else {
	            mac->setDestAddr(MacAddress::BROADCAST_ADDRESS);
	        }

	        mac->encapsulate(pktToSend_WSM->dup());

		}
		else if ((pktToSend_IP = dynamic_cast<inet::Packet*>(pktRaw)) != NULL){
            ASSERT(pktToSend_IP);
            lastAC = AC_BK;
            lastIPPkt = pktToSend_IP;

//            pktToSend_IP->getDataLength();
            std::cout << pktToSend_IP->getDataLength() << endl;
            //send the packet
            auto macAddressReq = pktToSend_IP->getTag<MacAddressReq>();
            auto destAddress = macAddressReq->getDestAddress();
            if (destAddress != MacAddress::UNSPECIFIED_ADDRESS) {
               mac->setDestAddr(destAddress);
            }
            else {
               mac->setDestAddr(MacAddress::BROADCAST_ADDRESS);
            }

            mac->encapsulate( pktToSend_IP->dup() );

		}
		else{
		    error("nextMacEvent: wrong type of packet from queue!");
		}
...
//		mac->encapsulate(pktToSend->dup());
		...
		PhyControlMessage* controlInfo = NULL;
		if (pktToSend_WSM != NULL){
		    controlInfo = dynamic_cast<PhyControlMessage *>(pktToSend_WSM->getControlInfo());
		}
...
//        std::cout << mac->getBitLength() << endl;
		...
		if ((!useSCH) || (timeLeftInSlot() > sendingDuration)) {
			...
			bool checkWSMPktUnicast;
	        if (pktToSend_WSM != NULL){
	            checkWSMPktUnicast = (pktToSend_WSM->getRecipientAddress() != MacAddress::UNSPECIFIED_ADDRESS);
	        }

	        bool checkIPPktUnicast;
	        if (pktToSend_IP != NULL){
	            checkIPPktUnicast = (pktToSend_IP->getTag<MacAddressReq>()->getDestAddress() != MacAddress::UNSPECIFIED_ADDRESS);
	        }

			if ((checkWSMPktUnicast || checkIPPktUnicast) && useAcks) {
...
void Mac1609_4::handleUpperMsg(cMessage* msg) {
    ...
    auto thisMsgWSM = dynamic_cast<WSM*>(msg);
    auto thisMsgPkt = dynamic_cast<inet::Packet*>(msg);

    if (thisMsgWSM != NULL) {
	    ac = mapUserPriority(thisMsgWSM->getUserPriority());

	    DBG_MAC << "Received a WSM message from upper layer for channel "
	            << thisMsgWSM->getChannelNumber() << " Access Category (Priority):  "
	            << ac << std::endl;

	    if (thisMsgWSM->getChannelNumber() == Channels::CCH) {
	        chan = type_CCH;
	    }
	    else {
	        ASSERT(useSCH);
	        thisMsgWSM->setChannelNumber(mySCH);
	        chan = type_SCH;
	    }

	    if ((thisMsg = dynamic_cast<cPacket*>(thisMsgWSM)) != NULL){
	        num = myEDCA[chan]->queuePacket(ac,thisMsg);
	    }
	    else{
	        error("WaveMac can't convert a WSM message!");
	    }
	}
	else if (thisMsgPkt != NULL){

	    chan = type_CCH;
	    ac = AC_BK;

        DBG_MAC << "Received an IP message from upper layer for channel "
                << chan << " Access Category (Priority):  "
                << ac << std::endl;
//        DBG_MAC << " " << thisMsgPkt-> << std::endl;

        if ((thisMsg = dynamic_cast<cPacket*>(thisMsgPkt)) != NULL){
            num = myEDCA[chan]->queuePacket(ac,thisMsg);
        }
        else{
            error("WaveMac can't convert an IP-Packet message!");
        }
	}
	else{
	    error("WaveMac can't identify a message!");
	}
...
void Mac1609_4::handleLowerControl(cMessage* msg) {
	if (msg->getKind() == MacToPhyInterface::PHY_RX_START) {
		...
		if (!dynamic_cast<Mac80211Ack*>(lastMac.get())) {
		    ...
		    if (lastCPkt)
		        myEDCA[activeChannel]->postTransmit(lastAC, lastCPkt, useAcks);
		    else
		        error("lastCPkt is empty!");
//		    if (lastWSM)
//		        myEDCA[activeChannel]->postTransmit(lastAC, lastWSM, useAcks);
//		    else if (lastIPPkt)
//	            myEDCA[activeChannel]->postTransmit(lastAC, lastIPPkt, useAcks);
...
void Mac1609_4::handleLowerMsg(cMessage* msg) {
    ...
    DBG_MAC << "Received frame name= " << macPkt->getName()
            << ", kind= " << macPkt->getKind()
            << ", frame's " << std::endl;
    DBG_MAC << "src=" << macPkt->getSrcAddr() << std::endl;
    DBG_MAC << "dst=" << macPkt->getDestAddr() << std::endl;
    DBG_MAC << "myAddr=" << myMacAddr << std::endl;

    cPacket* innerPkt = dynamic_cast<cPacket*>( macPkt->decapsulate() );
    ...
    if (dest == myMacAddr) {
        ...
        else if ( dynamic_cast<WSM*>(innerPkt) ) {
            unique_ptr<WSM> wsm(check_and_cast<WSM*>(innerPkt));
            wsm->setControlInfo(new PhyToMacControlInfo(res));
            handleUnicast(std::move(wsm));
        }
        else if ( dynamic_cast<inet::Packet*>(innerPkt) ){
            unique_ptr<inet::Packet> ip(check_and_cast<inet::Packet*>(innerPkt));
            ip->setControlInfo(new PhyToMacControlInfo(res));
            handleUnicast(std::move(ip));
        }
    }
    else if (dest == MacAddress::BROADCAST_ADDRESS) {
        statsReceivedBroadcasts++;
        if ( dynamic_cast<WSM*>(innerPkt) ){
            unique_ptr<WSM> wsm(check_and_cast<WSM*>(innerPkt));
            wsm->setControlInfo(new PhyToMacControlInfo(res));
            sendUp(wsm.release());
        }
        else if ( dynamic_cast<inet::Packet*>(innerPkt) ){
//            inet::Packet* ip = check_and_cast<inet::Packet*>(innerPkt);
            unique_ptr<inet::Packet> ipPkt(check_and_cast<inet::Packet*>(innerPkt));

            DBG_MAC << "Received an " << ipPkt->getName()
                    << " packet, send it to up layer." << std::endl;

//            ip->setControlInfo(new PhyToMacControlInfo(res));
            ipPkt->addTagIfAbsent<DispatchProtocolReq>()->setProtocol(&Protocol::ipv6);
            ipPkt->addTagIfAbsent<PacketProtocolTag>()->setProtocol(&Protocol::ipv6);
            if (interfaceEntry)
                ipPkt->addTagIfAbsent<InterfaceInd>()->setInterfaceId(interfaceEntry->getInterfaceId());

            sendUp(ipPkt.release());
//            sendUp(ip);
...
int Mac1609_4::EDCA::queuePacket(t_access_category ac, cPacket* msg) {
    ...
    if (maxQueueSize && myQueues[ac].queueMix.size() >= maxQueueSize) {
        delete msg;
        return -1;
    }
    myQueues[ac].queueMix.push(msg);
    return myQueues[ac].queueMix.size();
}
...
//	WaveShortMessage* pktToSend = NULL;
	...
	for (auto iter = myQueues.rbegin(); iter != myQueues.rend(); iter++) {
		if (iter->second.queueMix.size() != 0 && !iter->second.waitForAck) {
			if (idleTime >= iter->second.aifsn* SLOTLENGTH_11P + SIFS_11P && iter->second.txOP == true) {
				...
				if (pktToSend == NULL) {
					...
					pktToSend = iter->second.queueMix.front();
				}
				else {
...
simtime_t Mac1609_4::EDCA::startContent(simtime_t idleSince,bool guardActive) {
	...
	for (auto&& p : myQueues) {
		...
		if (edcaQueue.queueMix.size() != 0 && !edcaQueue.waitForAck) {
...
void Mac1609_4::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
	...
	for (auto&& p : myQueues) {
		auto &accessCategory = p.first;
		auto &edcaQueue = p.second;
		if ((edcaQueue.currentBackoff != 0 || edcaQueue.queueMix.size() != 0) && !edcaQueue.waitForAck) {
...
void Mac1609_4::EDCA::postTransmit(t_access_category ac, cPacket* msg, bool useAcks) {
    ...
    WSM* wsm = nullptr;
    inet::Packet* ip = nullptr;

    bool holBlocking;
    bool isWSM = (wsm = dynamic_cast<WSM*>(msg)) ? true : false;
    bool isIP = (ip = dynamic_cast<inet::Packet*>(msg)) ? true : false;

    if (isWSM){
        holBlocking = (wsm->getRecipientAddress() != MacAddress::UNSPECIFIED_ADDRESS) && useAcks;
    }
    else if (isIP){
        holBlocking = (ip->getTag<MacAddressReq>()->getDestAddress() != MacAddress::UNSPECIFIED_ADDRESS) && useAcks;
    }
	...
	if (holBlocking) {
		...
		delete myQueues[ac].queueMix.front();

		myQueues[ac].queueMix.pop();
...
void Mac1609_4::handleUnicast(unique_ptr<WSM> wsm) {
	if (useAcks) {
...
void Mac1609_4::handleUnicast(unique_ptr<inet::Packet> ip) {
    if (useAcks) {
        inet::MacAddress senderAddr = ip->getTag<MacAddressReq>()->getSrcAddress();
        sendAck(senderAddr, ip->getTreeId());
    }

    if (handledUnicastToApp.find(ip->getTreeId()) == handledUnicastToApp.end()) {
        handledUnicastToApp.insert(ip->getTreeId());
        DBG_MAC << "Received a data packet addressed to me." << std::endl;
        statsReceivedPackets++;
        sendUp(ip.release());
    }
...
void Mac1609_4::handleAck(const Mac80211Ack* ack) {
	...
	for (auto&& p : myEDCA[chan]->myQueues) {
		auto &accessCategory = p.first;
		auto &edcaQueue = p.second;
		if (edcaQueue.queueMix.size() > 0 && edcaQueue.waitForAck && (edcaQueue.waitOnUnicastID == ack->getMessageId())) {
			cPacket* pkt = edcaQueue.queueMix.front();
//            auto wsm = edcaQueue.queueMix.front();
//            boost::shared_ptr<WaveShortMessage*> wsm = edcaQueue.queueMix.front();
			edcaQueue.queueMix.pop();
			delete pkt;
...
void Mac1609_4::handleRetransmit(t_access_category ac) {
	...
	if(myEDCA[type_CCH]->myQueues[ac].queueMix.size() == 0) {
		throw cRuntimeError("Trying retransmission on empty queue...");
	}
	cPacket* appPkt = myEDCA[type_CCH]->myQueues[ac].queueMix.front();
	bool contend = false;
	...
	if (!retriesExceeded) {
		...
		// enough tries!
		myEDCA[type_CCH]->myQueues[ac].queueMix.pop();
		if (myEDCA[type_CCH]->myQueues[ac].queueMix.size() > 0) {
...
Mac1609_4::EDCA::EDCAQueue::~EDCAQueue() {
	while (!queueMix.empty()) {
		delete queueMix.front();
		queueMix.pop();
	}
...
//    ie->setInterfaceToken(token);
    ...
    // address
    ie->setMacAddress(myMacAddr);
//    std::cout << "IE: myMacAddr: " << myMacAddr << std::endl;

    ie->setInterfaceToken(myMacAddr.formInterfaceIdentifier());

    ie->setMtu(par("mtu"));
    // capabilities
    ie->setBroadcast(true);
    ie->setMulticast(true);
    ie->setPointToPoint(false);
