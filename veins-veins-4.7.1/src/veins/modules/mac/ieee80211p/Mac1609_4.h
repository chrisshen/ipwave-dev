...
#include "veins/modules/messages/WSM_m.h"
...
//						std::queue<boost::any> queueMix;
                        std::queue<cPacket*> queueMix;
...
//				std::map<t_access_category, EDCAQueue> myQueuesMix;
	...
	protected:
		...
		void handleUnicast(std::unique_ptr<WSM> wsm);
        void handleUnicast(std::unique_ptr<inet::Packet> wsm);
...
//		WaveShortMessage* lastWSM;
		...
		/*Chris*/
		inet::InterfaceEntry *interfaceEntry = nullptr;    // nullptr if no InterfaceTable or node is down

		inet::MacAddress myMacAddr;

//		inet::PacketWAVE* packet;

//		inet::Packet* packet;

        /** @brief pointer to last sent inet packet */
		inet::Packet* lastIPPkt;

        /** @brief pointer to last sent c packet */
        cPacket* lastCPkt;
